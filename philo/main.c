...
static enum e_errors	parse_input(int argc, char **argv, t_sim *sim)
{
	...
	if (readarg(argv[1], &sim->args.number_of_philos) || readtime(argv[2],
			&sim->args.time_to_die) || readtime(argv[3], &sim->args.time_to_eat)
		|| readtime(argv[4], &sim->args.time_to_sleep) || (argc == 6
			&& readarg(argv[5], &sim->args.number_of_eat_to_finish)))
...
static enum e_errors	initialize(t_sim *sim)
{
	...
	sim->forks_locks = ft_calloc(sim->args.number_of_philos,
			sizeof(*sim->forks_locks));
	sim->forks_states = ft_calloc(sim->args.number_of_philos,
